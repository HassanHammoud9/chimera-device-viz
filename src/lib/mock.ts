...
export async function fetchDeviceById(id: string): Promise<DeviceDetails | null> {
  await new Promise(r => setTimeout(r, 150));
  const d = base.find(x => x.id === id);
  if (!d) return null;

  const rand = (n: number) => Math.max(0, Math.min(100, Math.round(n + (Math.random()*10-5))));
  const trail = (v: number) => Array.from({length: 20}, (_, i) => rand(v + Math.sin(i/2)*8));
  ...
  return {
    ...
    logs: [
      ...
      ...(d.status === "offline"
          ? [{ ts: new Date(Date.now()-120000).toISOString(), level: "error", msg: "Agent unreachable" } as const]
          : []),
    ],
