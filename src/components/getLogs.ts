// Returns logs if present, otherwise generates realistic mock logs for UI demo
import { DeviceDetails } from "@/lib/types";
export type LogEntry = { ts: string; level: string; msg: string };
export function getLogs(data: DeviceDetails | null, loading: boolean): LogEntry[] {
  if (loading) {
  ...
  if (data.status === "offline") base[2] = { ts: String(now), level: "error", msg: "Device is offline." };
  return base;
}
